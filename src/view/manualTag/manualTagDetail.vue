                ...
                <div class="m-b-20">
                  ...
                  <div class="m-t-18 add-tags-wrap">
                    <template v-for="(item, index) in parent.addSelectTags">
                      <span :key="item.value + index" class="el-tag el-tag--small">{{ item.value }}<i class="el-tag__close el-icon-close" @click="delSearchSelectVal(index, parent)"></i></span>
                    </template>
                  </div>
                </div>
              </template>
                        ...
                        <el-tab-pane :label="'已选选项(' + parent.selectedOptions.length + ')'" name="second">
                          <div class="slected-opyions">
                            <template v-for="(item, index) in parent.selectedOptions">
                              <span :key="item.name + index" class="el-tag el-tag--small">{{ item.name }}<i class="el-tag__close el-icon-close" @click="delallSelectVal(index, item, parent)"></i></span>
                            </template>
                          </div>
                        </el-tab-pane>
                      </el-tabs>
                        ...
                        <el-tab-pane :label="'已选选项(' + parent.citySelectedOptions.length + ')'" name="second">
                          <div class="slected-opyions city-slected-opyions">
                            <template v-for="(item, index) in parent.citySelectedOptions">
                              <span :key="index" class="el-tag el-tag--small">{{ item.selected }}<i class="el-tag__close el-icon-close" @click="delcitySelectVal(index, item, parent)"></i></span>
                            </template>
                          </div>
                        </el-tab-pane>
                      </el-tabs>
        ...
        .then(res => {
          ...
          if (resData.errorCode == 1) {
            ...
            if (parseInt(resData.result.sourceFlag) === 0) {
              ...
              console.log(that[resData.result.templateCode].options);
            }
            // 如果通过 url 获取
